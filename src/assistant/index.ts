...
const humanActionProxy = (PAGE_ACCESS_TOKEN: string) => (
    ...
    return SendMarkSeen(PAGE_ACCESS_TOKEN)(userID)
        ...
        .catch(err => {
            console.error(
                "[examples/kkpoon-assistant] message handle error: " + err
            );
        ...
        .then(result =>
            SendTypingOff(PAGE_ACCESS_TOKEN)(userID).then(() => result)
        );
};
...
const handleImageMessage = (PAGE_ACCESS_TOKEN: string) => (
    ...
    if (att.payload && att.payload.sticker_id) {
        ...
        return SendTextMessage(PAGE_ACCESS_TOKEN)(userID)(
            "Sorry, I can't get the image"
        ).then(
            () =>
                "[examples/kkpoon-assistant] response with error on getting image"
        );
    }
};
